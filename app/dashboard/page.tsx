    ...
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {showMore && (
        <div style={{
          ...
          borderRadius: 12, padding: 6, minWidth: 180, zIndex: 100,
...
// ── AI Progress Panel ───────────────────────────────────────────────
function AIProgress({ visible }: { visible: boolean }) {
  const stages = [
    { icon: "🧠", label: "Thinking...",     done: true  },
    { icon: "📖", label: "Reading Files...", done: true  },
    { icon: "✏️",  label: "Editing Navbar", done: true  },
    { icon: "⚡",  label: "Generating Code",done: false, active: true },
    { icon: "🔍",  label: "Running Tests",  done: false },
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12, fontWeight: 700, color: T.gold, marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
        <div style={{ width: 6, height: 6, borderRadius: "50%", background: T.gold, animation: "pulse-dot 1s infinite" }} />
        Krypton AI is working...
      </div>
      {stages.map((s, i) => (
        <div key={i} style={{
          display: "flex", alignItems: "center", gap: 8,
          padding: "5px 0", opacity: s.done || (s as any).active ? 1 : 0.3,
        }}>
          <span style={{ fontSize: 14 }}>{s.icon}</span>
          <span style={{
            fontSize: 12.5,
            color: s.done ? T.green : (s as any).active ? T.gold : T.muted,
            fontWeight: (s as any).active ? 700 : 400,
          }}>{s.label}</span>
          {s.done && <span style={{ marginLeft: "auto", color: T.green, fontSize: 11 }}>✓</span>}
          {(s as any).active && (
            <span style={{ marginLeft: "auto", display: "flex", gap: 3 }}>
              {[0,1,2].map(j => (
                <span key={j} style={{
                  ...
                  animation: `dot-pulse 1.2s ${j * 0.2}s infinite`,
                }} />
              ))}
            </span>
          )}
...
export default function DashboardPage() {
  ...
  const init = async () => {
    ...
    await Promise.all([fetchProjects(session.user.id), fetchCredits(session.user.id), fetchActivities(session.user.id)]);
    setLoading(false);
  };

  const fetchProjects = async (uid: string) => {
    const { data } = await supabase.from("projects")
      .select("id, title, prompt, html_code, created_at, updated_at, status")
      .eq("user_id", uid)
      .order("updated_at", { ascending: false });
    setProjects((data || []).map(p => ({ ...p, starred: false })));
  };
        ...
        <div style={{ display: "flex", maxWidth: 1400, margin: "0 auto", gap: 0 }}>
                  ...
                  <div key={project.id} className="project-card" style={{
                    ...
                    <div style={{ padding: "14px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                        ...
                        }}>{project.title || "Untitled"}</h3>
                        <button onClick={() => toggleStar(project.id)} style={{
                          background: "none", border: "none", cursor: "pointer",
                          fontSize: 14, color: project.starred ? "#FFD700" : "#444",
                      ...
                      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        <button onClick={() => router.push(`/create?id=${project.id}`)}
                          style={{
                        ...
                        <button onClick={() => { const w = window.open("","_blank"); if(w){w.document.write(project.html_code);w.document.close();} }}
                          style={{
                            flex: 1, padding: "5px 8px", borderRadius: 7,
                        ...
                        <button onClick={() => router.push(`/create?id=${project.id}&tab=chat`)}
                          style={{
                            flex: 1, padding: "5px 8px", borderRadius: 7,
          ...
          <div style={{
            ...
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 16 }}>
              ...
              <div style={{ marginTop: 12, padding: "10px", background: "#161616", borderRadius: 8 }}>
            ...
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 16 }}>
              ...
              {[
                ...
                { label: "Drafts",         value: projects.filter(p => p.status === "draft" || !p.status).length, icon: "📝" },
            ...
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                <p style={{ fontSize: 13, fontWeight: 700, margin: 0 }}>AI Activity</p>
              ...
              {activities.length > 0 && (
                <button onClick={() => router.push("/settings?tab=billing")} style={{
                  ...
                  borderRadius: 8, color: T.muted, fontSize: 11.5,
                  cursor: "pointer",
            ...
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 16 }}>
              ...
              {projects.slice(0, 4).map(p => (
                <div key={p.id}
                  ...
                  <div style={{
                    width: 28, height: 28, borderRadius: 6, background: "#161616",
                    display: "flex", alignItems: "center", justifyContent: "center",
        ...
        {confirmDelete && (
          <div onClick={() => setConfirmDelete(null)} style={{
            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <p style={{ color: T.muted, fontSize: 13, marginBottom: 24 }}>
                This cannot be undone. Project will be permanently deleted.
              </p>
