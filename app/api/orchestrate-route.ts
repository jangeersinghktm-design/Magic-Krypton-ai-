  ...
  for (const [fn, name] of [[callClaude, "claude"],[callOpenAI,"openai"],[callGemini,"gemini"]] as const) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        // One bounded retry only for transient rate-limit (429) responses —
        // a short delay often clears a momentary spike on the same provider
        // before falling through to the next one. Any other error (timeout,
        // 5xx, missing key) falls through immediately, unchanged from before.
...
// Duplicate-generation guard is now distributed (lib/generation-lock.ts) — see acquireGenerationLock/releaseGenerationLock below.
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // no "phase" events are sent. Harmless: client ignores unknown event types.
      ...
      try {
        ...
        if (authedUserId && !(await acquireGenerationLock(authedUserId))) {
...
${viBoost}
        ...
        if (authedUserId) await releaseGenerationLock(authedUserId);
        finish();
      }
    },
  });
