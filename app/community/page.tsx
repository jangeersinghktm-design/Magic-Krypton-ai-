...
function CommunityCard({ item, onLike, onUse }: { item: CommunityProject; onLike: (id: string) => void; onUse: (item: CommunityProject) => void }) {
  ...
  return (
    <div
      ...
      style={{
        background: T.card, border: `1px solid ${hovered ? "rgba(245,197,66,0.3)" : T.border}`,
      ...
      <div style={{ height: 160, background: "#111", position: "relative", overflow: "hidden" }}>
        ...
        {item.is_featured && (
          <div style={{ position: "absolute", top: 10, left: 10 }}>
            <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", background: G, color: "#000", borderRadius: 20 }}>
              ⭐ Featured
            </span>
          </div>
        )}
        ...
        {hovered && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, animation: "fadeIn 0.15s ease" }}>
            <button onClick={() => { const w = window.open("","_blank"); if(w && item.projects?.html_code){w.document.write(item.projects.html_code);w.document.close();} }}
              style={{ padding: "8px 16px", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8, color: "#fff", fontSize: 12, cursor: "pointer" }}>
              👁 Preview
            </button>
            <button onClick={() => onUse(item)} style={{ padding: "8px 16px", background: G, border: "none", borderRadius: 8, color: "#000", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              Use →
            </button>
          </div>
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", gap: 12 }}>
            <button onClick={() => onLike(item.id)} style={{
              ...
              color: item.liked ? "#ef4444" : T.muted, fontSize: 12,
...
export default function CommunityPage() {
  ...
  const init = async () => {
    ...
    let likedIds: string[] = [];
    if (session?.user?.id) {
      const { data: likes } = await supabase.from("project_likes").select("community_project_id").eq("user_id", session.user.id);
  ...
  const loadMyProjects = async () => {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    const { data } = await supabase.from("projects").select("id, title, html_code, prompt").eq("user_id", session.user.id).order("created_at", { ascending: false }).limit(20);
    setMyProjects(data || []);
  };
  ...
  const handleUse = async (item: CommunityProject) => {
      ...
      <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{
          ...
          <p style={{ color: T.muted, fontSize: 15, marginBottom: 24, maxWidth: 500, margin: "0 auto 24px" }}>
            Discover amazing projects built with Krypton AI. Share yours and inspire others!
          </p>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => { loadMyProjects(); setShowShare(true); }} style={{
              padding: "10px 20px", background: G, border: "none", borderRadius: 10,
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px" }}>
          ...
          {!loading && filtered.length > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 16 }}>
              {filtered.map(item => (
                <CommunityCard key={item.id} item={item} onLike={handleLike} onUse={handleUse} />
              ))}
            </div>
          )}
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ textAlign: "center", padding: "60px 24px", background: T.card, border: `1px solid ${T.border}`, borderRadius: 18 }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>🌍</div>
              <p style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>No projects yet</p>
              <p style={{ color: T.muted, fontSize: 14, marginBottom: 24 }}>Be the first to share your project!</p>
              ...
              {[
                ...
                <div key={field.label} style={{ marginBottom: 12 }}>
                  ...
                  <input value={field.value} onChange={e => field.setter(e.target.value)} placeholder={field.placeholder}
                    style={{ width: "100%", background: "#161616", border: `1px solid ${T.border}`, borderRadius: 8, color: T.text, padding: "9px 12px", fontSize: 13, outline: "none", marginTop: 6, boxSizing: "border-box" as const }} />
                </div>
              ))}
