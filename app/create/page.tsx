...
function classifyIntent(text: string, hasProject: boolean): Intent {
  ...
  if (hasProject && (EDIT_VERBS.test(t) || UI_ELEMENTS.test(t))) return "EDIT";
...
function CreatePageInner() {
  ...
  const dispatchMessage = (p:string, mode:AIMode) => {
    ...
    const intent = classifyIntent(p, !!result);
  ...
  const runChatOnly = async (message:string) => {
    addMsg({role:"user",type:"text",content:message});
    setLoading(true);
    const thinkId = addMsg({role:"ai",type:"thinking",content:"",isActive:true,phases:[
  ...
  const runExplain = async (message:string, targetFile:string) => {
    ...
    try {
      ...
      const res = await fetch("/api/chat-assistant", {
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":`Bearer ${session.access_token}`},
        body:JSON.stringify({message, history, mode:"explain", fileName:targetFile, fileContent:fileContent.slice(0,12000)}),
        signal:AbortSignal.timeout(60000),
      });
      const data = await res.json();
      updateMsg(thinkId,{isActive:false,phases:[{agent:"Reading",icon:"○",action:"Answered",pct:100,done:true,status:"done"}]});
  ...
  const runPlanning = async (message:string) => {
    ...
    try {
      ...
      const res = await fetch("/api/chat-assistant", {
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":`Bearer ${session.access_token}`},
        body:JSON.stringify({message, history, mode:"planning"}),
        signal:AbortSignal.timeout(60000),
      });
      const data = await res.json();
