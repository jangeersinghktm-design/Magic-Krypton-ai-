...
// Full project context object — passed to AI on every edit request
interface ProjectMemory {
  projectName:    string;
  originalPrompt: string;
  colorSystem:    string;   // CSS variables extracted from HTML
  fonts:          string;   // Google Fonts detected
  sections:       string;   // Page sections (h1, h2 content)
  navigation:     string;   // Nav links
  colorScheme: {
    backgrounds:  string;
    textColors:   string;
    accentColors: string;
  ...
  editHistory:    string[]; // All previous user edit requests
  codeLines:      number;
  lastUpdated:    Date;
}
...
function buildProjectMemory(
  ...
  if (!html) return prevMemory || {
    projectName, originalPrompt, colorSystem: "", fonts: "",
    sections: "", navigation: "", colorScheme: { backgrounds:"", textColors:"", accentColors:"" },
    layout: { hasNavbar:false, hasFooter:false, hasSidebar:false, isFullWidth:true, isDarkTheme:false },
    editHistory: [], codeLines: 0, lastUpdated: new Date(),
  };

  // Extract CSS custom properties
  const cssVarMatches = html.match(/--[\w-]+\s*:\s*[^;}{]+/g) || [];
  const colorSystem = cssVarMatches.slice(0,25).join("; ").slice(0,600);

  // Extract colors
  const bgColors  = [...new Set((html.match(/background(?:-color)?\s*:\s*(#[0-9a-fA-F]{3,8}|rgba?\([^)]+\))/g) || []).slice(0,6))];
  const txtColors = [...new Set((html.match(/(?<![a-z-])color\s*:\s*(#[0-9a-fA-F]{3,8})/g) || []).slice(0,6))];
  const accColors = [...new Set((html.match(/(?:border|outline|gradient)[^:]*:\s*[^;]*(#[0-9a-fA-F]{3,8})/g) || []).slice(0,4))];

  // Extract fonts
  const fontMatches = html.match(/family=([^&"'\s\)]+)/g) || [];
  const fonts = [...new Set(fontMatches.map(f => f.replace("family=","").split(":")[0].replace(/\+/g," ")))].join(", ") || "System fonts";

  // Extract sections
  const h1s = (html.match(/<h1[^>]*>([^<]+)<\/h1>/gi) || []).map(h=>h.replace(/<[^>]+>/g,"").trim());
  const h2s = (html.match(/<h2[^>]*>([^<]+)<\/h2>/gi) || []).map(h=>h.replace(/<[^>]+>/g,"").trim()).slice(0,8);
  const sections = [...h1s, ...h2s].join(" | ").slice(0,300);

  // Extract navigation
  const navLinks = (html.match(/<(?:a|li)[^>]*>([A-Za-z][^<]{1,25})<\/(?:a|li)>/gi) || [])
    .map(a=>a.replace(/<[^>]+>/g,"").trim()).filter(t=>t.length>2 && t.length<30)
    .filter((v,i,a)=>a.indexOf(v)===i).slice(0,12);

  // Layout detection
  const hasNavbar  = /<nav|class="nav|id="nav/.test(html);
  const hasFooter  = /<footer|class="footer/.test(html);
  const hasSidebar = /sidebar|side-bar/.test(html);
  const isDarkTheme = /#[0-1][0-9a-f]{5}/.test(bgColors.join("")) ||
    /background.*#0[0-9a-f]/.test(html.slice(0,2000));

  // Edit history from conversation
  const allEdits = messages
  ...
  return {
    ...
    colorScheme: {
      backgrounds: bgColors.join(", ").slice(0,200),
      textColors:  txtColors.join(", ").slice(0,200),
      accentColors: accColors.join(", ").slice(0,200),
    },
    layout: { hasNavbar, hasFooter, hasSidebar, isFullWidth:!hasSidebar, isDarkTheme },
    editHistory: allEdits,
...
function formatMemoryForAI(mem: ProjectMemory | null): string {
  ...
  return `
╔═ KRYPTON PROJECT MEMORY ════════════════════════╗
║ Project: ${mem.projectName}
║ Original request: ${mem.originalPrompt.slice(0,100)}
║ Code: ${mem.codeLines} lines | Theme: ${mem.layout.isDarkTheme?"Dark":"Light"}
║ Has: ${[mem.layout.hasNavbar&&"Navbar",mem.layout.hasFooter&&"Footer",mem.layout.hasSidebar&&"Sidebar"].filter(Boolean).join(", ")||"Custom layout"}
╠═ DESIGN SYSTEM (MUST PRESERVE) ════════════════╣
║ Fonts: ${mem.fonts}
║ CSS Variables: ${mem.colorSystem.slice(0,300)}
║ Backgrounds: ${mem.colorScheme.backgrounds}
║ Text colors: ${mem.colorScheme.textColors}
╠═ PAGE STRUCTURE ════════════════════════════════╣
║ Sections: ${mem.sections.slice(0,200)}
║ Navigation: ${mem.navigation}
╠═ EDIT HISTORY ══════════════════════════════════╣
${mem.editHistory.map((e,i)=>`║ ${i+1}. ${e.slice(0,80)}`).join("\n")||"║ No previous edits"}
╚═══════════════════════════════════════════════╝

CRITICAL RULES:
- Preserve ALL colors, fonts, and design system above
- Only change what the user explicitly requested
- Never remove existing sections unless asked
...
function CreatePageInner() {
  ...
  const runAgentFlow = async (userPrompt: string) => {
    ...
    // Build and store project memory for future edits
            ...
            <div style={{
              ...
              {loading && (
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  ...
                  <span style={{ fontSize:11, color:C.gold }}>Working on it…</span>
                </div>
              )}
              {!loading && result && (
            ...
            <div style={{
              ...
              <div style={{
                ...
                <textarea
                  ...
                  placeholder={loading ? "Working on it…" : result ? "Describe a change..." : "Describe what you want to build..."}
                  rows={2}
                  disabled={loading}
                  style={{
                ...
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:7, paddingTop:7, borderTop:`1px solid rgba(255,255,255,.04)` }}>
                  <span style={{ fontSize:11, color:"#2a2a2a" }}>Krypton Intelligence Engine</span>
