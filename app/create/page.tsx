...
function CreatePage() {
  ...
  const saveVersion = async (code: string, message: string, type = "auto") => {
    ...
    const { data } = await supabase.from("project_versions").insert({
      project_id: projectId, code_snapshot: { "index.html": code },
      message, type, version_number: versions.length + 1, size_bytes: new Blob([code]).size,
    }).select().single();
  ...
  const triggerGenerate = async (overridePrompt?: string) => {
    ...
    if (remaining < CREDIT_COSTS.new_project) {
  ...
  const triggerEdit = async (editPrompt: string) => {
    ...
    if (remaining < CREDIT_COSTS.ai_edit) {
      ...
      <div style={{
              ...
              <div style={{
                ...
                <div style={{ padding: "10px 12px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 10px", borderRadius: 20, background: "linear-gradient(135deg,#F5D800,#00CC44)", color: "#000" }}>
                      {planLabel.toUpperCase()}
                    </span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
                    <span style={{ fontSize: 12, color: "#555" }}>Credits</span>
                    <span onClick={() => { router.push("/settings?tab=billing"); setShowDropdown(false); }}
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0, marginLeft: "auto" }}>
            ...
            {result && (
              <>
                ...
                {!isMobile && (
                  <button onClick={handleDownload} style={{ padding: "5px 10px", background: "#161616", border: "1px solid #1c1c1c", borderRadius: 7, color: "#9ca3af", cursor: "pointer", fontSize: 11 }}>Export</button>
                )}
              </>
            )}
          </div>
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          <div style={{
              ...
              {messages.map((msg) => (
                <div key={msg.id} className="msg-enter" style={{ display: "flex", flexDirection: "column", alignItems: msg.role === "user" ? "flex-end" : "flex-start", gap: 3 }}>
                  <div style={{
                    ...
                    background: msg.role === "user" ? "linear-gradient(135deg,rgba(245,197,66,0.2),rgba(245,197,66,0.1))" : "#161616",
                    ...
                    {msg.credits && (
                      <div style={{ marginTop: 8, fontSize: 10.5, color: "#444", display: "flex", alignItems: "center", gap: 4 }}>
            ...
            {/* Error */}
            {error && (
              <div style={{ margin: "0 12px 8px", padding: "8px 12px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, color: "#ef4444", fontSize: 12 }}>
                ⚠ {error}
              </div>
            )}
            ...
            <div style={{ padding: "10px 12px", borderTop: "1px solid #1c1c1c", background: "#0C0C0C", flexShrink: 0 }}>
              {result && (
                <div style={{ marginBottom: 6, fontSize: 10.5, color: "#555", display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <div style={{ flex: 1, display: isMobile ? (activeTab === "preview" ? "flex" : "none") : "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {rightTab === "preview" && (
              <div style={{ flex: 1, background: "#111", overflow: "hidden" }}>
                {result ? (
                  <iframe key={result.length} srcDoc={result} style={{ width: "100%", height: "100%", border: "none" }} sandbox="allow-scripts allow-same-origin" title="Preview" />
                ) : (
                  <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, color: "#333" }}>
            ...
            {rightTab === "export" && (
              <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
                ...
                {[
                  ...
                  { label: "Open in New Tab", icon: "↗", action: () => { const w = window.open("", "_blank"); if (w) { w.document.write(result); w.document.close(); } } },
                ].map(opt => (
                  <button key={opt.label} onClick={opt.action} disabled={!result} style={{ width: "100%", marginBottom: 10, padding: "12px 16px", background: "#0d0d0d", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, color: result ? "#e8e8e8" : "#444", fontSize: 13, cursor: result ? "pointer" : "not-allowed", display: "flex", alignItems: "center", gap: 10, textAlign: "left" }}>
