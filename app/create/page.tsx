...
function CreatePageInner() {
  ...
  const loadProject = async (id:string, uid:string) => {
    const {data:{session:loadSession}} = await supabase.auth.getSession();
    if (!loadSession) return;
    const res = await fetch(`/api/deploy?projectId=${id}&userId=${uid}`, {
    ...
    try {
      ...
      const process=(chunk:string)=>{
        ...
        if (em[1]==="error"){
          ...
          if (data.code === "COST_GUARD_ABORT") {
            // Distinct from the credit-purchase modal — this isn't a
            // "buy more credits" situation, it's "this specific request
            // was too expensive to safely proceed". Shows the real,
            // detailed message (with actual estimated cost figures)
            // rather than a generic crash.
            throw new Error(data.message || "This request was too expensive to safely generate. Please try a shorter or simpler prompt.");
          }
      ...
      try {
        ...
        //
        // Retry with backoff BEFORE falling back to /api/generate — this is
        // the fix for the false-402 race: orchestrate may simply still be
        // mid-pipeline (e.g. inside the Claude-timeout-then-OpenAI-retry
        // gap) when the client's connection drops, in which case checking
        // only once and immediately falling back finds nothing (project
        // not saved yet) and triggers a second, credit-charging generation.
        // Waiting gives the still-running orchestrate request genuine time
        // to finish and save first.
        let recent: { id: string; html_code: string } | null = null;
        const RETRY_DELAYS_MS = [3000, 7000, 15000, 20000]; // ~45s total extra wait
        for (let attempt = 0; !recent && attempt <= RETRY_DELAYS_MS.length; attempt++) {
          if (attempt > 0) await new Promise(r => setTimeout(r, RETRY_DELAYS_MS[attempt - 1]));
          // Real progress feedback — connection was lost, so no more SSE
          // phase/heartbeat events arrive; without this the UI would show
          // the last frozen message ("Planning project structure...") for
          // the entire retry window, looking exactly like a hang.
          updateMsg(thinkId, { phases: [
            { agent: "Reconnecting", icon: "🔄", action: `Connection lost — checking if your build finished (attempt ${attempt + 1}/${RETRY_DELAYS_MS.length + 1})...`, pct: 60, done: false, status: "running" },
          ], isActive: true });
        ...
        if (recent?.html_code) {
          ...
          receivedProjectType = detected.isGame ? "game" : "website"; // was left empty on recovery — silently skipped multi-page generation
        } else {
          updateMsg(thinkId, { phases: [
            { agent: "Reconnecting", icon: "🔄", action: "Still not found — starting a fresh attempt...", pct: 65, done: false, status: "running" },
          ], isActive: true });
          ...
          else if (fd.code === "DUPLICATE_GEN") {
            // The fallback itself confirms the original generation is still
            // genuinely active (same shared lock) — this is NOT a failure,
            // it means "wait longer", so retry the recovery-check a bounded
            // number of extra times rather than showing a generic error.
            for (let extra = 0; extra < 3 && !html; extra++) {
              updateMsg(thinkId, { phases: [
                { agent: "Reconnecting", icon: "🔄", action: `Your original build is still finishing — waiting (${extra + 1}/3)...`, pct: 70, done: false, status: "running" },
              ], isActive: true });
              await new Promise(r => setTimeout(r, 10000));
              const { data: laterRecent } = await supabase
                .from("projects")
                .select("id, html_code")
                .eq("user_id", session.user.id)
                .eq("prompt", userPrompt)
                ...
                .limit(1)
                .maybeSingle();
              if (laterRecent?.html_code) { html = laterRecent.html_code; savedPid = laterRecent.id; credUsed = 0; }
            }
          }
  ...
  const runEdit = async (editPrompt:string, targetFile?:string) => {
    ...
    // Real, explicit (not vague NLP) routing to Single Component AI
    // Regeneration — only fires when the message clearly names one of
    // the 12 known components AND a real, saved project exists. Falls
    // through to the existing /api/chat flow for everything else
    // (freeform requests, unsaved projects, non-component edits) — the
    // old flow is never removed, just no longer the ONLY path.
    const COMPONENT_EDIT_PATTERN = /^(change|update|edit|regenerate|redo|rewrite)\s+(the\s+)?(hero|navbar|pricing|footer|faq|features|stats|testimonials|cta|portfolio|dashboard|ecommerce)\b/i;
    const componentMatch = editPrompt.trim().match(COMPONENT_EDIT_PATTERN);
    if (componentMatch && projectId && !isGameProject) {
      try {
        const rc = await fetch("/api/regenerate-component", {
          method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${session.access_token}` },
          body: JSON.stringify({ projectId, componentType: componentMatch[3].toLowerCase(), editInstruction: editPrompt }),
          signal: AbortSignal.timeout(60000),
        });
        const rcData = await rc.json();
        if (rc.ok && rcData.html && rcData.changed) {
          newHtml = rcData.html;
        }
      } catch { /* falls through to the existing chat flow below */ }
    }

    let d1: any = null;
    try {
      if (newHtml) {
        // Single Component Regeneration already produced the result —
        // skip the general chat flow entirely for this message.
      } else {
