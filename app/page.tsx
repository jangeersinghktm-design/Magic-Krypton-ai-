...
export default function HomePage() {
  ...
  const handleDeleteProject = async (id: string) => {
    ...
    // Optimistic: remove immediately, keep the removed item + its index so
    // we can restore it exactly where it was if the server call fails.
    const removedIndex = projects.findIndex(p => p.id === id);
    const removed = projects[removedIndex];
    setProjects(prev => prev.filter(p => p.id !== id));
    const { error } = await supabase.from("projects").delete().eq("id", id);
    if (error) {
      // Rollback — restore the item to its original position
      setProjects(prev => {
        const next = [...prev];
  ...
  const handleDuplicateProject = async (p: any) => {
    if (!user) return;
    // Optimistic: show a temporary copy immediately (client-side temp id),
    // replace it with the real server row on success, remove it on failure.
    ...
    setProjects(prev => [optimisticCopy, ...prev]);

    const { data, error } = await supabase.from("projects").insert({
      user_id: user.id,
      title: optimisticCopy.title,
      name: optimisticCopy.name,
      prompt: p.prompt,
      html_code: p.html_code,
      status: "completed",
      created_at: optimisticCopy.created_at,
      updated_at: optimisticCopy.updated_at,
    }).select().single();
    ...
    if (data) setProjects(prev => prev.map(x => x.id === tempId ? data : x)); // swap temp for real
  };
  ...
  const handleRenameProject = async (p: any) => {
    ...
    // Optimistic: update immediately, revert to the exact prior name on failure.
    setProjects(prev => prev.map(x => x.id === p.id ? { ...x, title: trimmed, name: trimmed } : x));
    const { error } = await supabase.from("projects").update({ title: trimmed, name: trimmed }).eq("id", p.id);
    if (error) {
      setProjects(prev => prev.map(x => x.id === p.id ? { ...x, title: currentName, name: currentName } : x));
