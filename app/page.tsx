...
<div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
  ...
  <div style={{ position: "absolute", inset: 0, backgroundImage: "linear-gradient(rgba(245,197,66,0.018) 1px, transparent 1px), linear-gradient(90deg, rgba(245,197,66,0.018) 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
      ...
      {/* SIDEBAR */}
      <div style={{ width: "240px", height: "100vh", background: "#0C0C0C", borderRight: "1px solid #1c1c1c", display: "flex", flexDirection: "column", overflow: "hidden", transition: "transform 0.25s ease", flexShrink: 0, position: isMobile ? "fixed" : "relative", left: 0, top: 0, zIndex: 50, transform: sidebarOpen ? "translateX(0)" : "translateX(-100%)" }}>
        ...
        <div style={{ padding: "12px", borderTop: "1px solid #1c1c1c", display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "linear-gradient(135deg, #F5C542, #00D084)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: 700, color: "#080808", flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid #1c1c1c", display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: "8px" }}>
            ...
            <button onClick={() => router.push("/create")} style={{ padding: "7px 12px", background: "linear-gradient(135deg, #F5C542, #00D084)", border: "none", borderRadius: "9px", color: "#080808", fontSize: "12px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
              + New
            </button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "1.5rem 1rem" }}>
          <div style={{ width: "100%", maxWidth: "680px" }}>
            ...
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(24px, 5vw, 48px)", fontWeight: 800, textAlign: "center", marginBottom: "10px", lineHeight: 1.15 }}>
              Got an idea,{" "}
              <span style={{ background: "linear-gradient(135deg, #F5C542, #00D084)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                {firstName}?
              </span>
            </h1>
            ...
            <div style={{ background: "#101010", border: "1px solid rgba(245,197,66,0.15)", borderRadius: "18px", padding: "14px" }}>
              <textarea
                value={prompt}
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "8px", borderTop: "1px solid #1c1c1c", paddingTop: "10px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    {showBuildDropdown && (
                      <div style={{ position: "absolute", bottom: "44px", right: 0, background: "#141414", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "6px", zIndex: 100, minWidth: "120px" }}>
                        {["Website", "App", "Game", "Tool"].map((type) => (
                          <button key={type} onClick={() => { setBuildType(type); setShowBuildDropdown(false); }} style={{ width: "100%", textAlign: "left", padding: "9px 12px", background: type === buildType ? "#1c1c1c" : "none", border: "none", color: type === buildType ? "#F5C542" : "#9ca3af", fontSize: "13px", cursor: "pointer", borderRadius: "8px" }}>
                            {type}
                          </button>
                        ))}
                      </div>
                  ...
                  <button onClick={handleVoice} style={{ width: "36px", height: "36px", borderRadius: "50%", background: listening ? "linear-gradient(135deg, #F5C542, #00D084)" : "#2a2a2a", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}>
                    {listening ? (
                  ...
                  <button onClick={handleGenerate} disabled={!prompt.trim()} style={{ width: "36px", height: "36px", borderRadius: "50%", background: prompt.trim() ? "linear-gradient(135deg, #F5C542, #00D084)" : "#1a1a1a", border: "none", cursor: prompt.trim() ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "10px", marginTop: "2rem" }}>
              {[
                ...
                <button key={card.title} onClick={() => router.push(card.path)} style={{ background: "#101010", border: "1px solid rgba(245,197,66,0.1)", borderRadius: "14px", padding: "1rem", textAlign: "left", cursor: "pointer", transition: "all 0.2s" }}
                  onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#F5C542"; e.currentTarget.style.boxShadow = "0 0 20px rgba(245,197,66,0.08)"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(245,197,66,0.1)"; e.currentTarget.style.boxShadow = "none"; }}
                >
                  <div style={{ fontSize: "18px", marginBottom: "6px", background: "linear-gradient(135deg, #F5C542, #00D084)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{card.icon}</div>
