...
export default function KryptonSidebar() {
  ...
  const [openMenus, setOpenMenus] = useState<string[]>(["Create"]); // default open
  ...
  return (
    <>
      <style>{`
        ...
        .ks-item.active {
          background: linear-gradient(135deg, rgba(245,197,66,0.15), rgba(0,208,132,0.08));
          color: var(--gold);
        ...
        .ks-sub-item svg { opacity: 0.8; flex-shrink: 0; }
        ...
        .ks-user-info { flex: 1; min-width: 0; }
        .ks-user-name {
          ...
          {navItems.map((item) => {
            ...
            return (
              <div key={item.label} className="ks-item-wrap">
                ...
                {hasChildren ? (
                  <div
                    className={`ks-item ${active ? "active" : ""}`}
                    onClick={() => toggle(item.label)}
                  >
                    <span className="ks-item-left">
                      {item.icon}
                      {item.label}
                    </span>
                    <span className={`ks-chevron ${isOpen ? "open" : ""}`}>
                      {icons.chevron}
                    </span>
                  </div>
                ) : (
                    ...
                    <div className="ks-sub-inner">
                      {item.children!.map((child) => (
                        <Link
                          key={child.href}
                          href={child.href}
                          className={`ks-sub-item ${isActive(child.href) ? "active" : ""}`}
                        >
                          {child.icon}
                          {child.label}
                        </Link>
